...
from pydantic import BaseModel, root_validator, model_validator, Field, ValidationError
    ...
    def validate_refresh_token_fields(data: dict) -> None:
        if not data.get("refresh_token"):
            ...
            raise CustomValidationException(
                loc=("refresh_token",),
                msg="refresh token field is required"
            )
    ...
    def validate_authorization_code_fields(data: dict) -> None:
        ...
        for require_field in require_fields:
            if not data.get(require_field):
                raise CustomValidationException(
                    loc=(require_field,),
                    msg=f"{require_field} field is required"
                )
