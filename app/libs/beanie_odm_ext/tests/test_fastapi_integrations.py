...
class TestFastApiIntegration:
    ...
    @pytest.mark.asyncio
    async def test_api_create_product_success(
    ...
    @pytest.mark.asyncio
    async def test_api_transaction_rollback(
        self, mocker: MockerFixture, api_client: AsyncClient, fast_api_app
    ) -> None:
        # Spy on the driver to verify the 'abort' signal was sent

        spy_abort = mocker.spy(AsyncClientSession, "abort_transaction")
        spy_commit = mocker.spy(AsyncClientSession, "commit_transaction")

        # Act: This endpoint raises an Exception and is decorated with @atomic
    ...
    @pytest.mark.asyncio
    async def test_api_get_product_flow(self, mocker: MockerFixture, api_client: AsyncClient) -> None:
        # 1. Seed
        spy_session_enter = mocker.spy(WireSession, "__aenter__")
        spy_session_exit = mocker.spy(WireSession, "__aexit__")
