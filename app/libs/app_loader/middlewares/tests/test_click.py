...
def get_apps_context_fixture(base_dir: Path):
    return {
        "app_a": {
            "name": "app_a",
            "module_path": "app.libs.app_loader.middlewares.tests.fixture_click.app_a",
            "folder_path": base_dir.joinpath("app/libs/app_loader/middlewares/tests/fixture_click/app_a"),
        },
        "app_b": {
            "name": "app_b",
            "module_path": "app.libs.app_loader.middlewares.tests.fixture_click.app_b",
            "folder_path": base_dir.joinpath("app/libs/app_loader/middlewares/tests/fixture_click/app_b"),
        },
    }
...
class TestClickCommandLoader:
    ...
    def test_command_loader(self, base_dir: Path, command_loader: ClickCommandLoader) -> None:
        ...
        context["apps"] = get_apps_context_fixture(base_dir)
