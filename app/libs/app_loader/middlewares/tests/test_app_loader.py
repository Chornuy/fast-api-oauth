...
def get_mocked_success_flow(base_dir: Path) -> list:
    return [get_fixture_path(base_dir, "app_a"), get_fixture_path(base_dir, "app_b")]


def get_expected_result_on_success_flow(base_dir: Path):
    return {
        "app_a": {
            "name": "app_a",
            "module_path": "app.libs.app_loader.middlewares.tests.fixture_app_loader.app_a",
            "folder_path": base_dir.joinpath("app/libs/app_loader/middlewares/tests/fixture_app_loader/app_a"),
        },
        "app_b": {
            "name": "app_b",
            "module_path": "app.libs.app_loader.middlewares.tests.fixture_app_loader.app_b",
            "folder_path": base_dir.joinpath("app/libs/app_loader/middlewares/tests/fixture_app_loader/app_b"),
        },
    }
